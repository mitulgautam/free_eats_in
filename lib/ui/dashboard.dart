...
class _DashboardState extends State<Dashboard> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(title: Text("FreeEats"), centerTitle: true),
        floatingActionButtonLocation: FloatingActionButtonLocation.centerDocked,
        floatingActionButton: RaisedButton(
            onPressed: () => showDialog(
                context: context,
                builder: (context) => _currentPage == 0
                    ? AddNewFoodPointEvent(isUpdate: false)
                    : AddHelpEvent(isUpdate: false),
                barrierDismissible: false),
            color: Themes.DARK_BROWN_COOKIE,
            child: Row(mainAxisSize: MainAxisSize.min, children: <Widget>[
              Icon(Icons.add, color: Colors.white),
              Text(_currentPage == 0 ? "Add Event" : "Help Event",
        ...
        body: SafeArea(
            child: IndexedStack(index: _currentPage, children: _widget)));
  }
}
